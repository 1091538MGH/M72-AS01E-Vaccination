functions {
  ...
  real get_smooth_TPT(real t, real TPT_max_pre2021, real TPT_max_post2021, real TPT_min_fraction, real TPT_future_trend) {
    ...
    if (t <= 2021) {  // Fixed: was t <= 2014 twice
  ...
  // Vaccination coverage function with waning
  real get_vaccination_coverage(real t, real vaccination_start_time, real vaccine_coverage, 
                               real revaccination_interval, real vaccine_waning_rate) {
    if (t < vaccination_start_time) {
      return 0.0;
    }
    
    real years_since_start = t - vaccination_start_time;
    
    // Build up coverage over first 2 years, then maintain with revaccination and waning
    real target_coverage = vaccine_coverage;
    
    if (years_since_start <= 2.0) {
      // Initial rollout phase
      return target_coverage * (years_since_start / 2.0);
    } else {
      // Steady state with revaccination and waning
      // coverage maintained by revaccination schedule
      real effective_coverage = target_coverage * (1.0 - vaccine_waning_rate * revaccination_interval / 2.0);
      return fmax(0.0, fmin(target_coverage, effective_coverage));
    }
  }
  ...
  vector tb_ode(real t, vector y, vector theta, real[] x_r, int[] x_i) {
    ...
    real kappa = x_r[7];
    real chi = x_r[8];
    real vaccine_efficacy = x_r[9];
    real vaccine_coverage = x_r[10];
    real revaccination_interval = x_r[11];
    real vaccination_start_time = x_r[12];
    real vaccine_waning_rate = x_r[13];
    ...
    // Calculate vaccination effect (only for adults)
    real current_vaccine_coverage = get_vaccination_coverage(t + 2000, vaccination_start_time, 
                                                           vaccine_coverage, revaccination_interval, 
                                                           vaccine_waning_rate);
    real vaccine_protection = current_vaccine_coverage * vaccine_efficacy;
    ...
    // Apply vaccination to theta1 (adults only): reduce progression rate by vaccine protection
    real theta1 = theta1_base * (1.0 - TPT_eff_a) * (1.0 - vaccine_protection);
    ...
    // Children: get infected by adults, don't transmit (no vaccination effect)
    ...
    // Adults: get infected by adults, can transmit (vaccination affects theta1)
...
parameters {
  ...
  real<lower=0.01, upper=0.5> theta_param_base;
  real<lower=0.01, upper=0.4> theta1_base;
  real<lower=0.0, upper=0.3> rho;
  real<lower=0.0, upper=0.15> w;
  real<lower=0.5, upper=4> beta; 
  real<lower=0.0, upper=0.2> alpha;  
  real<lower=0.0, upper=0.2> alpha_1; 
...
model {
  // Improved priors
  theta_param_base ~ lognormal(log(0.3), 0.3);
  theta1_base ~ lognormal(log(0.5), 0.3);
  rho ~ lognormal(log(0.03), 0.3);
  w ~ lognormal(log(0.08), 0.3);
  beta ~ normal(2.5, 0.8);
  ...
  L ~ lognormal(log(10000), 0.4);
  L1 ~ lognormal(log(450000), 0.3);

  DOTS_effects ~ beta(3, 5); 
  ACT_effects ~ beta(3, 7);
  TPT_c_max_pre2021 ~ beta(7, 3);
  TPT_a_max_pre2021 ~ beta(9, 1);
  TPT_c_max_post2021 ~ beta(1, 2);
  TPT_a_max_post2021 ~ beta(6, 4);
  
  TPT_min_fraction ~ beta(1, 9);
  TPT_future_trend_c ~ normal(0, 0.8);
  TPT_future_trend_a ~ normal(0, 0.8);
